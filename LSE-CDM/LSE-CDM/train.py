...
from models.Autoencoder import SparseEnhancer
from models.DNN import DNN
from models.Tenc import Tenc
...
parser.add_argument('--seq_length', type=int, default=50, help='maximum number of items in user sequence')
parser.add_argument('--class_free', type=bool, default=False, help='use classifier-free')
...
parser.add_argument('--diffuser_type',type=str, default='cad', help='the type of diffusion model: Tenc')
parser.add_argument('--p', type=float, default=0.1, help='p of classifier-free guidance scheme')
parser.add_argument('--w', type=float, default=1, help='w of classifier-free guidance scheme ')
...
# ===== 在 imports 后或脚本合适位置添加新的超参SLEM =====
parser.add_argument('--slem_topk', type=int, default=5, help='top-K neighbors for SLEM')
parser.add_argument('--lambda_slem', type=float, default=0.05, help='weight of SLEM regularization')
parser.add_argument('--slem_hidden', type=int, default=32, help='hidden size for slem MLP')
...
# train_data, valid_y_data, test_y_data, train_seqs, train_seq_lens, n_user, n_item = data_utils.data_load(train_path, valid_path, test_path, args.seq_length)
...
train_data, valid_y_data, test_y_data, train_seqs, train_seq_lens, train_inter_counts, n_user, n_item = \
    data_utils.data_load(train_path, valid_path, test_path, args.seq_length)

train_dense = torch.FloatTensor(train_data.A)
train_dataset = data_utils.FullTrainDataset(train_dense, train_seqs, train_seq_lens, train_inter_counts)
# train_dataset = data_utils.FullTrainDataset(train_dense, train_seqs, train_seq_lens)
...
test_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=data_utils.collate_fn)
...
diffusion = gd.GaussianDiffusion(mean_type, args.noise_schedule, \
                                 args.noise_scale, args.noise_min, args.noise_max, args.steps, args.w, device).to(device)

### Build MLP ###
latent_size = in_dims[-1]
mlp_out_dims = eval(args.mlp_dims) + [latent_size]
mlp_in_dims = mlp_out_dims[::-1]
mlp_dims = eval(args.mlp_dims)[0]
# model = DNN(mlp_in_dims, mlp_out_dims, args.emb_size, time_type="cat", norm=args.norm, act_func=args.mlp_act_func).to(
#     device)
model = Tenc(mlp_dims, n_item, args.seq_length, args.dropout_rate, args.diffuser_type, device) # 注意seq_size 暂设置为n_item 后续数据集需要处理
slem = SparseEnhancer(latent_size, hidden=args.slem_hidden, topk=args.slem_topk, device=device).to(device)
param_num = 0
...
slem_num = sum([param.nelement() for param in slem.parameters()])
param_num = AE_num + mlp_num + diff_num + slem_num

print("Number of parameters:", param_num)
params_AE_SLEM = list(Autoencoder.parameters()) + list(slem.parameters())
if args.optimizer1 == 'Adagrad':
    optimizer1 = optim.Adagrad(
        params_AE_SLEM, lr=args.lr1, initial_accumulator_value=1e-8, weight_decay=args.wd1)
elif args.optimizer1 == 'Adam':
    optimizer1 = optim.Adam(params_AE_SLEM, lr=args.lr1, weight_decay=args.wd1)
elif args.optimizer1 == 'AdamW':
    optimizer1 = optim.AdamW(params_AE_SLEM, lr=args.lr1, weight_decay=args.wd1)
elif args.optimizer1 == 'SGD':
    optimizer1 = optim.SGD(params_AE_SLEM, lr=args.lr1, weight_decay=args.wd1)
elif args.optimizer1 == 'Momentum':
    optimizer1 = optim.SGD(params_AE_SLEM, lr=args.lr1, momentum=0.95, weight_decay=args.wd1)
...
def evaluate(data_loader, data_te, mask_his, topN):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            t_batch = batch['dense'].to(device)
            t_seqs = batch['seq'].to(device)
            t_seq_lens = batch['len'].to(device)
            t_real_lens = batch['real_len'].to(device)

            # mask map
            his_data = mask_his[e_idxlist[batch_idx * args.batch_size:batch_idx * args.batch_size + len(t_batch)]]

            _, batch_latent, _ = Autoencoder.Encode(t_batch)
            batch_latent = slem.inference(batch_latent, t_batch, t_real_lens)
            h = model.get_h(t_seqs, t_seq_lens)
            batch_latent_recon = diffusion.p_sample(model, batch_latent, h, args.sampling_steps, args.sampling_noise)
...
for epoch in range(1, args.epochs + 1):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        batch_cate, batch_latent, vae_kl = Autoencoder.Encode(t_batch)

        # ==== 应用 SLEM ====
        z_enhanced, l_slem = slem(batch_latent, t_batch, t_real_lens)

        # x_start = model.cacu_x(batch_latent)
        if args.class_free:
            h = model.cacu_h(t_seqs, t_seq_lens, args.p)
        else:
            h = model.get_h(t_seqs, t_seq_lens)
        terms = diffusion.training_losses(model, batch_latent, h, args.reweight)
    ...
    if epoch % 5 == 0 :
        ...
        if valid_results[1][1] >= best_recall:  # recall@20 as selection
            ...
            # torch.save(model,
            #            '{}{}_{}lr1_{}lr2_{}wd1_{}wd2_bs{}_cate{}_in{}_out{}_lam{}_dims{}_emb{}_{}_steps{}_scale{}_min{}_max{}_sample{}_reweight{}_{}_Tenc.pth' \
            #            .format(save_path, args.dataset, args.lr1, args.lr2, args.wd1, args.wd2, args.batch_size,
            #                    args.n_cate, \
            #                    args.in_dims, args.out_dims, args.lamda, args.mlp_dims, args.emb_size, args.mean_type,
            #                    args.steps, \
            #                    args.noise_scale, args.noise_min, args.noise_max, args.sampling_steps, args.reweight,
            #                    args.log_name))
            # torch.save(Autoencoder,
            #            '{}{}_{}lr1_{}lr2_{}wd1_{}wd2_bs{}_cate{}_in{}_out{}_lam{}_dims{}_emb{}_{}_steps{}_scale{}_min{}_max{}_sample{}_reweight{}_{}_Tenc_AE.pth' \
            #            .format(save_path, args.dataset, args.lr1, args.lr2, args.wd1, args.wd2, args.batch_size,
            #                    args.n_cate, \
            #                    args.in_dims, args.out_dims, args.lamda, args.mlp_dims, args.emb_size, args.mean_type,
            #                    args.steps, \
