...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
            steps, w, device, history_num_per_term=10, beta_fixed=False):
    ...
    def p_sample(self, model, x_start, h, steps, sampling_noise=False):
        ...
        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, h, t)
            if sampling_noise:
    ...
    def training_losses(self, model, x_start, h, reweight=False):
    ...
    def p_mean_variance(self, model, x, h, t):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B, )
        model_output = model(x, h, t) # 条件引导
        # model_output = (1 + self.w) * model(x, h, t) - self.w * model.forward_uncon(x, t) # 无分类器引导
